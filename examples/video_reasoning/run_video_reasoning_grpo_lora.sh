#!/bin/bash
# Video Reasoning GRPO Training Script (LoRA Version)
# 使用 LoRA 进行参数高效微调，减少显存和训练时间
# 运行前确保在 verl 项目根目录
...
# ===== LoRA 配置 =====
LORA_RANK=16                          # LoRA 秩，越大参数越多，效果越好，但显存占用也越大
LORA_ALPHA=32                         # LoRA alpha，通常设为 2*rank
LORA_TARGET_MODULES="all-linear"      # 应用 LoRA 的模块，all-linear 表示所有线性层

# ===== 训练参数（8卡配置，LoRA 可以用更大的 batch size）=====
TRAIN_BATCH_SIZE=4                    # LoRA 显存占用小，可以增大 batch size
...
EXPERIMENT_NAME="video_reasoning_grpo_lora_r${LORA_RANK}_${TIMESTAMP}"
...
# ===== STEP 2: Run training with LoRA =====
echo "===== Step 2: Starting GRPO training with LoRA (rank=$LORA_RANK) ====="
...
nohup python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='video-reasoning-grpo-lora' \
    ...
    custom_reward_function.path=pkg://verl.utils.reward_score.video_reasoning \
