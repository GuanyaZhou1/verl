...
if [ "$ADV_ESTIMATOR" = "gdpo" ]; then
    ...
    # GRPO 模式：使用原有命名
    EXPERIMENT_NAME="Qwen3-VL-8B-Instruct-multiturn_spatial_dapo_holmes_genbs${GEN_BATCH_SIZE}_trainbs${TRAIN_BATCH_SIZE}_ep${TOTAL_EPOCHS}_lr${LEARNING_RATE}_ans${ANSWER_WEIGHT}_bbox${BBOX_WEIGHT}_fmt${FORMAT_WEIGHT}_seg${SEGMENT_WEIGHT}_klcoef${KL_LOSS_COEF}_filtergroups${ENABLE_FILTER_GROUPS}_stdthres${FILTER_GROUPS_STD_THRESHOLD}_cliphi${CLIP_RATIO_HIGH}_topp${TOP_P}_temp${TEMPERATURE}_mf${INITIAL_VIDEO_MAX_FRAMES}_mp${INITIAL_VIDEO_MAX_PIXELS}_0326"
fi
