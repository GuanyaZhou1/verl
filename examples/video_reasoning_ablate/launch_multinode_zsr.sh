...
RAY_TEMP_DIR="/tmp/ray_$(whoami)"  # 用户独立的 Ray 临时目录，避免多用户 session 冲突
EXTRA_ARGS=""
PROJECT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
# 远程节点上激活 conda 再执行 ray/python（SSH 非交互 shell 不会自动激活 conda）
# 注意：base conda Python 3.13 标准库不完整（encodings/aliases.py 缺失），
# 无法使用 conda activate。改为直接将 verl_clone/bin 加入 PATH。
VERL_ENV_PATH="/share_data/gyzhou/anaconda3/envs/verl_clone"
REMOTE_PREFIX="unset PYTHONPATH PYTHONHOME && export PYTHONNOUSERSITE=1 && export PATH=${VERL_ENV_PATH}/bin:\$PATH && export CONDA_PREFIX=${VERL_ENV_PATH} && "
...
# ssh_cmd with a timeout (for commands that may hang, e.g. ray stop)
ssh_cmd_timeout() {
    local seconds="$1"
    local node="$2"
    shift 2
    if [ -n "$SSH_USER" ]; then
        timeout "$seconds" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ServerAliveInterval=5 -o ServerAliveCountMax=3 "${SSH_USER}@${node}" "$@"
    else
        timeout "$seconds" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ServerAliveInterval=5 -o ServerAliveCountMax=3 "${node}" "$@"
    fi
}
...
cleanup() {
    ...
    for node in "${NODE_LIST[@]}"; do
        ...
        ssh_cmd_timeout 30 "$node" "${REMOTE_PREFIX}ray stop --force 2>/dev/null" &>/dev/null || true
    done
...
for node in "${NODE_LIST[@]}"; do
    echo "  Stopping Ray on $node ..."
    ssh_cmd_timeout 30 "$node" "${REMOTE_PREFIX}ray stop --force 2>/dev/null; \
...
# 确保 GCS 端口已释放
if ssh_cmd "$HEAD_NODE" "ss -tlnp | grep -q ':$RAY_PORT '" 2>/dev/null; then
    echo "  [WARN] Port $RAY_PORT still in use on $HEAD_NODE, killing occupying process..."
    ssh_cmd "$HEAD_NODE" "fuser -k $RAY_PORT/tcp 2>/dev/null" || true
    sleep 2
fi
# 清理旧 session 目录，避免 GCS 初始化读到残留状态
for node in "${NODE_LIST[@]}"; do
    ssh_cmd "$node" "rm -rf $RAY_TEMP_DIR 2>/dev/null; mkdir -p $RAY_TEMP_DIR" || true
done

# 启动 head，带重试（GCS 偶尔因时序问题首次失败）
HEAD_STARTED=false
for attempt in 1 2 3; do
    echo "  Attempt $attempt: Starting Ray head on $HEAD_NODE ..."
    if ssh_cmd "$HEAD_NODE" "${REMOTE_PREFIX}$NCCL_ENVS && cd $PROJECT_DIR && ray start --head --port=$RAY_PORT \
        --temp-dir=$RAY_TEMP_DIR --dashboard-host=0.0.0.0 --num-gpus=$GPUS_PER_NODE --disable-usage-stats" 2>&1; then
        HEAD_STARTED=true
        break
    fi
    echo "  [WARN] Ray head start failed, retrying in 5s..."
    ssh_cmd_timeout 10 "$HEAD_NODE" "${REMOTE_PREFIX}ray stop --force 2>/dev/null" || true
    ssh_cmd "$HEAD_NODE" "pkill -9 -f gcs_server 2>/dev/null; rm -rf /tmp/ray/session_latest 2>/dev/null" || true
    sleep 5
done

if [ "$HEAD_STARTED" != "true" ]; then
    echo "ERROR: Failed to start Ray head after 3 attempts"
    exit 1
fi

# 健康检查：验证 GCS 进程在运行且端口在监听
echo "  Verifying Ray head health..."
sleep 2
HEAD_HEALTHY=false
for hc in 1 2 3 4 5; do
    # 检查 GCS 进程是否存活
    if ! ssh_cmd "$HEAD_NODE" "pgrep -f gcs_server" &>/dev/null; then
        echo "  [WARN] GCS server process not found (check $hc/5)"
        sleep 3
        continue
    fi
    # 检查端口是否在监听
    if ! ssh_cmd "$HEAD_NODE" "ss -tlnp | grep -q ':$RAY_PORT '" 2>/dev/null; then
        echo "  [WARN] Port $RAY_PORT not listening (check $hc/5)"
        sleep 3
        continue
    fi
    # 检查 ray status 能否连通
    if ssh_cmd "$HEAD_NODE" "${REMOTE_PREFIX}RAY_ADDRESS=$HEAD_NODE:$RAY_PORT ray status" &>/dev/null; then
        HEAD_HEALTHY=true
        echo "  [OK] Ray head healthy: GCS running, port $RAY_PORT listening, ray status OK"
        break
    else
        echo "  [WARN] ray status failed (check $hc/5)"
        sleep 3
    fi
done

if [ "$HEAD_HEALTHY" != "true" ]; then
    echo "ERROR: Ray head started but health check failed. GCS may have crashed."
    echo "  Check GCS logs: ssh $HEAD_NODE cat $RAY_TEMP_DIR/ray/session_latest/logs/gcs_server.err"
    exit 1
fi
echo ""
...
if [ $NNODES -gt 1 ]; then
    ...
    for i in $(seq 1 $((NNODES - 1))); do
        ...
        # 先检查 worker → head 端口连通性
        if ! ssh_cmd "$worker" "timeout 5 bash -c 'echo >/dev/tcp/$HEAD_NODE/$RAY_PORT' 2>/dev/null"; then
            echo "  [WARN] $worker cannot reach $HEAD_NODE:$RAY_PORT — port may be blocked"
        else
            echo "  [OK] $worker -> $HEAD_NODE:$RAY_PORT is reachable"
        fi

        # 启动 worker，捕获输出和返回码
        # 注意：不能让 set -e 在这里生效，否则 worker_rc=$? 永远执行不到
        worker_output=$(ssh_cmd "$worker" "${REMOTE_PREFIX}$NCCL_ENVS && cd $PROJECT_DIR && ray start --address=$HEAD_NODE:$RAY_PORT \
            --temp-dir=$RAY_TEMP_DIR --num-gpus=$GPUS_PER_NODE --disable-usage-stats 2>&1") && worker_rc=0 || worker_rc=$?
        echo "$worker_output"
        if [ $worker_rc -ne 0 ]; then
            echo "  [ERROR] ray start on $worker failed (exit code $worker_rc)"
            echo "  Check Ray logs on $worker: /tmp/ray/session_latest/logs/"
        fi
    done
    echo ""
fi
...
# 显式传递 ray.init(address=...) 参数，确保连接已有集群而非启动新集群
NCCL_HYDRA_ARGS="+ray_kwargs.ray_init.address=$HEAD_NODE:$RAY_PORT"
NCCL_HYDRA_ARGS="$NCCL_HYDRA_ARGS +ray_kwargs.ray_init.runtime_env.env_vars.NCCL_SOCKET_IFNAME=$NCCL_IFNAME"
...
ssh_cmd "$HEAD_NODE" "${REMOTE_PREFIX}cd $PROJECT_DIR && \
    ...
    bash examples/video_reasoning_ablate/run_video_reasoning_singleturn_nospatial.sh $NCCL_HYDRA_ARGS $EXTRA_ARGS"
