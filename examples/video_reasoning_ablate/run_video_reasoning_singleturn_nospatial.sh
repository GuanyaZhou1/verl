...
# =============================================================================
ulimit -n 65535
export PYTHONNOUSERSITE=1                # 避免 ~/.local 下的 antlr4 版本冲突
...
# 注意：多节点模式下由 launcher 管理 Ray 集群，此处不能启动/停止
...
MODEL_PATH="/data_gpu/zhengshurong/data/project/Qwen3-VL/qwen-vl-finetune/checkpoints/video/ablate_reasoning_format/Qwen3-VL-8B-Instruct-openo3video_stgr_singleturn_nospatial_7k-self_holmes_singleturn_nospatial_1k5-self_longvideoreason_singleturn_nospatial_5k3-sft-lr5e-5-bs32"
DATA_DIR="${DATA_DIR:-./long_video_data_ablate/single_nospatial_cot/video_holmes}"
CACHE_DIR="${CACHE_DIR:-./.cache_zsr}"
CONFIG_PATH="$(pwd)/examples/video_reasoning/config"
LOG_DIR="./logs_zsr_ablate"
...
SEGMENT_WEIGHT=${SEGMENT_WEIGHT:-0.5}        # segment 时间段匹配奖励权重 (0 = 不使用)
...
RESUME_MODE=auto                      # disable / resume_path / auto
...
if [ "$ADV_ESTIMATOR" = "gdpo" ]; then
    ...
    # GRPO 模式：使用原有命名
    EXPERIMENT_NAME="Qwen3-VL-8B-Instruct-singleturn_nospatial_dapo_holmes_genbs${GEN_BATCH_SIZE}_trainbs${TRAIN_BATCH_SIZE}_ep${TOTAL_EPOCHS}_lr${LEARNING_RATE}_ans${ANSWER_WEIGHT}_bbox${BBOX_WEIGHT}_fmt${FORMAT_WEIGHT}_seg${SEGMENT_WEIGHT}_klcoef${KL_LOSS_COEF}_filtergroups${ENABLE_FILTER_GROUPS}_stdthres${FILTER_GROUPS_STD_THRESHOLD}_cliphi${CLIP_RATIO_HIGH}_topp${TOP_P}_temp${TEMPERATURE}_mf${INITIAL_VIDEO_MAX_FRAMES}_mp${INITIAL_VIDEO_MAX_PIXELS}_0326"
fi

# 将 reward_logs 和 tensorboard_log 放到 checkpoint 目录下
CKPT_BASE="./checkpoints_zsr_ablate/${PROJECT_NAME}/${EXPERIMENT_NAME}"
