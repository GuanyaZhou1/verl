...
MODEL_PATH="/data_gpu/zhengshurong/data/project/Qwen3-VL/qwen-vl-finetune/checkpoints/video/ablate_reasoning_format/Qwen3-VL-8B-Instruct-openo3video_stgr_singleturn_nospatial_7k-self_holmes_multiturn_nospatial_1k5-self_longvideoreason_multiturn_nospatial_5k3-sft-lr5e-5-bs32"
DATA_DIR="${DATA_DIR:-./long_video_data_ablate/multiturn_nospatial/video_holmes}"
...
if [ "$ADV_ESTIMATOR" = "gdpo" ]; then
    ...
    # GRPO 模式：使用原有命名
    EXPERIMENT_NAME="Qwen3-VL-8B-Instruct-multiturn_nospatial_dapo_holmes_genbs${GEN_BATCH_SIZE}_trainbs${TRAIN_BATCH_SIZE}_ep${TOTAL_EPOCHS}_lr${LEARNING_RATE}_ans${ANSWER_WEIGHT}_bbox${BBOX_WEIGHT}_fmt${FORMAT_WEIGHT}_seg${SEGMENT_WEIGHT}_klcoef${KL_LOSS_COEF}_filtergroups${ENABLE_FILTER_GROUPS}_stdthres${FILTER_GROUPS_STD_THRESHOLD}_cliphi${CLIP_RATIO_HIGH}_topp${TOP_P}_temp${TEMPERATURE}_mf${INITIAL_VIDEO_MAX_FRAMES}_mp${INITIAL_VIDEO_MAX_PIXELS}_0326"
fi
