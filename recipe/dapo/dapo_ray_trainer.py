...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def compute_kl_related_metrics(self, batch: DataProto, metrics: dict, timing_raw: dict):
        ...
        # Check for bypass mode: skip recomputing old_log_probs, use rollout_log_probs directly
        rollout_corr_config = self.config.algorithm.get("rollout_correction", None)
        bypass_mode = rollout_corr_config and rollout_corr_config.get("bypass_mode", False)

        if bypass_mode and "rollout_log_probs" in batch.batch:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Skip in bypass mode (handled in compute_kl_related_metrics via apply_bypass_mode)
                    rollout_corr_config = self.config.algorithm.get("rollout_correction", None)
                    bypass_mode = rollout_corr_config and rollout_corr_config.get("bypass_mode", False)
                    if not bypass_mode and rollout_corr_config is not None and "rollout_log_probs" in batch.batch:
